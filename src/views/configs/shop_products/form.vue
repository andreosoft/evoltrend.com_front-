<template>
  <form1
    :breadcrumbs="[{url: '#/', title: 'Домой'}, {url: '#/configs/shop_products', title: 'Товары'}, {url: '', title: data.id ? data.name : 'Новый товар'}]"
    :loading="loading"
  >
    <template v-slot:title>{{data.id ? data.name : 'Новый товар'}}</template>
      ...
      if (confirm("Вы уверены, что хотите удалить?")) {
        ...
        axios({
          ...
          .then(response => {
            ...
            router.push({ name: "shop_products", params: {} });
          })
          .catch(error => {
            console.log(error);
          });
